  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        // Record view
        if (!user?.isAdmin) {
          await axios.post(`/api/admin/analytics/record-view/${productId}`);
        }
      } catch (err) {
      ...
      <div className="bg-white rounded-lg shadow-lg overflow-hidden">
        <div className="md:flex">
          ...
          <div className="md:w-1/2 p-6">
            <div className="mb-6">
              <h1 className="text-3xl font-bold text-gray-800 mb-2 font-playfair">{product.title}</h1>
              <p className="text-sm text-gray-500 mb-2">Category: {product.category}</p>
              {product.artist && (
                <p className="text-md text-gray-700 mb-2">Artist: {product.artist}</p>
              )}
              {product.creationYear && (
                <p className="text-md text-gray-700 mb-2">Year: {product.creationYear}</p>
              )}
            </div>
            ...
            <div className="mb-6">
              ...
              {product.dimensions && (product.dimensions.width || product.dimensions.height) && (
                <div className="mb-4">
                  ...
                  <p className="text-gray-600">
                    {product.dimensions.width && `Width: ${product.dimensions.width} ${product.dimensions.unit || 'cm'}`}
                    {product.dimensions.height && `, Height: ${product.dimensions.height} ${product.dimensions.unit || 'cm'}`}
                    {product.dimensions.depth && `, Depth: ${product.dimensions.depth} ${product.dimensions.unit || 'cm'}`}
                  </p>
                </div>
              )}
              ...
              <p className={`mb-4 font-medium ${
                ...
                product.stockQuantity <= 5 ? 'text-amber-600' : 'text-green-600'
              }`}>
